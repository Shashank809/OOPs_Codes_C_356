...
public:
    ...
    void setbalance(double b) //Added setter
    {
      balance=b;
    }
    ...
    {
        balance = balance + amt; //There was no semicolon
    }
...
int main()
{
    ...
    s1.setbalance(10000); //Added setter and changed syntax

    s1.display(); //no object was used

    Account a1(501,1000);//constructor didn't pass arguments

    PremiumSavings p2(1001, 5000, 5, 200); //wrong conversion request

    s1.deposit(-500);

    cout << p1.getinterestRate() << endl;

    //s1.~Savings(); not required
